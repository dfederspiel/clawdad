...
Write `/tmp/clawdad-diagnostics.json`. No paths, usernames, hostnames, or IP addresses.
...
**Yes**:
```bash
curl -s -X POST https://us.i.posthog.com/capture/ -H 'Content-Type: application/json' -d @/tmp/clawdad-diagnostics.json
rm /tmp/clawdad-diagnostics.json
```

**No**: `rm /tmp/clawdad-diagnostics.json`
...
4. `rm /tmp/clawdad-diagnostics.json`
