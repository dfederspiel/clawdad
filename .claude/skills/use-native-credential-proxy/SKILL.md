...
This skill replaces the OneCLI gateway with ClawDad's built-in credential proxy. Containers get credentials injected via a local HTTP proxy that reads from `.env` — no external services needed.
...
- macOS: `launchctl kickstart -k gui/$(id -u)/com.clawdad`
...
```bash
tail -20 logs/clawdad.log | grep "Credential proxy"
```
