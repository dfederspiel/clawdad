...
Example: delegate_to_agent({ agent: "analyst", message: "Please analyze the three jokes I just told and rate them." })`,
    {
      ...
      history_scope: z.enum(['none', 'recent', 'full']).optional().describe('How much prior group history the specialist sees alongside your delegation message. Unset (recommended): inferred from completion_policy — "final_response" gets "none" (fire-and-forget, no need for context), "retrigger_coordinator" gets "recent" (you\'ll synthesize the reply, so light context helps the specialist stay coherent). Set explicitly to override: "none" cuts cost on self-contained tasks; "recent" pulls the last ~3 messages; "full" pulls the last 10 (rare — your delegation message should usually carry the intent).'),
    },
    async (args) => {
      ...
      writeIpcFile(DELEGATIONS_DIR, {
        ...
        // Pass through what the coordinator gave us (or nothing). The
        // host applies the policy-aware default when unset (#137 Phase 3).
        historyScope: args.history_scope,
        sourceAgent: process.env.NANOCLAW_AGENT_NAME || 'default',
