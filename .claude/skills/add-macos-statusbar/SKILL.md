...
description: Add a macOS menu bar status indicator for ClawDad. Shows a bolt icon with a green/red dot indicating whether ClawDad is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows ClawDad's running status and lets the user
...
```bash
launchctl list | grep com.clawdad.statusbar
```
...
Create `~/Library/LaunchAgents/com.clawdad.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.clawdad.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.clawdad.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see ClawDad's status and control the service.
>
> - **Green dot** — ClawDad is running
> - **Red dot** — ClawDad is stopped
>
...
```bash
launchctl unload ~/Library/LaunchAgents/com.clawdad.statusbar.plist
rm ~/Library/LaunchAgents/com.clawdad.statusbar.plist
rm dist/statusbar
```
