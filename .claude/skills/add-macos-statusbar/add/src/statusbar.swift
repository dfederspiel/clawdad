...
class StatusBarController: NSObject {
    ...
    private let plistPath = "\(NSHomeDirectory())/Library/LaunchAgents/com.clawdad.plist"

    /// Derive the ClawDad project root from the binary location.
    ...
    private func setupStatusItem() {
        ...
        if let button = statusItem.button {
            if let image = NSImage(systemSymbolName: "bolt.fill", accessibilityDescription: "ClawDad") {
                image.isTemplate = true
                button.image = image
            } else {
                button.title = "⚡"
            }
            button.toolTip = "ClawDad"
        }
    }
    ...
    private func checkRunning() -> Bool {
        ...
        task.arguments = ["list", "com.clawdad"]
        let pipe = Pipe()
    ...
    private func updateMenu() {
        ...
        let label = isRunning ? "ClawDad is running" : "ClawDad is stopped"
    ...
    @objc private func restartService() {
        let uid = getuid()
        run("/bin/launchctl", ["kickstart", "-k", "gui/\(uid)/com.clawdad"])
        refresh(after: 3)
    }

    @objc private func viewLogs() {
        let logPath = "\(StatusBarController.projectRoot)/logs/clawdad.log"
