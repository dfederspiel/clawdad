---
name: add-emacs
description: Add Emacs as a channel. Opens an interactive chat buffer and org-mode integration so you can talk to ClawDad from within Emacs (Doom, Spacemacs, or vanilla). Uses a local HTTP bridge — no bot token or external service needed.
---

# Add Emacs Channel

This skill adds Emacs support to ClawDad, then walks through interactive setup.
...
- **Schedule tasks** — ask Andy to set a reminder or create a scheduled ClawDad task (e.g. "remind me tomorrow to review the PR")
...
```elisp
;; ClawDad — personal AI assistant channel
...
Either a stale ClawDad process is running, or 8766 is taken by another app.
...
Check:
1. ClawDad is running: `launchctl list | grep clawdad` (macOS) or `systemctl --user status clawdad` (Linux)
2. Emacs group is registered: `sqlite3 store/messages.db "SELECT * FROM registered_groups WHERE jid = 'emacs:default'"`
3. Logs show activity: `tail -50 logs/clawdad.log`

If the group is not registered, it will be created automatically on the next ClawDad restart.
...
If ClawDad is cloned elsewhere, update the `load`/`load-file` path in your Emacs config.
...
3. Remove the ClawDad block from your Emacs config file
...
6. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.clawdad` (macOS) or `npm run build && systemctl --user restart clawdad` (Linux)