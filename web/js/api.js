...
export const getTools = (runtime) => {
  const qs = [];
  if (runtime?.provider) qs.push(`provider=${encodeURIComponent(runtime.provider)}`);
  if (runtime?.model) qs.push(`model=${encodeURIComponent(runtime.model)}`);
