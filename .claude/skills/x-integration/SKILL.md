---
name: x-integration
description: X (Twitter) integration for ClawDad. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** ClawDad v1.0.0. Directory structure may change in future versions.
...
1. **ClawDad is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep clawdad (macOS) or systemctl --user status clawdad (Linux)
```
...
| `logs/clawdad.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into ClawDad, make the following modifications:
...
**Verify success:**
```bash
launchctl list | grep clawdad  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status clawdad
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/clawdad.log | tail -20

# Script errors
grep -i "error\|failed" logs/clawdad.log | tail -20
```
...
docker run clawdad-agent ls -la /app/src/skills/
```
